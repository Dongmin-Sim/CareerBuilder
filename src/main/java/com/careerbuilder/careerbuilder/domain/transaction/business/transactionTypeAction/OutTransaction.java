...
public class OutTransaction implements TypeTransaction {
    private final TransactionType type = TransactionType.OUT;
    ...
    public TransactionResponse action(TransactionRegisterRequest request) {
        ...
        stockBusiness.decreaseStockWithItemList(
    ...
    public void validateRequest(TransactionRegisterRequest request) {
        if (request.getFromLocation() == null) {
            throw new ApiException(TransactionError.VALIDATION_ERROR, "출고지가 필요합니다.");
        }
